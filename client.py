            ...
            if(numberBox['text']):
                if(int(numberBox['text']) not in flashNumberList):
                    if(platform.system() == 'Darwin'):
                        numberBox.config(state='disabled', disabledbackground='#f48fb1',
                            disabledforeground="white")
                    else:
                        numberBox.config(state='disabled', background='#f48fb1',
                            foreground="white")
...
def gameWindow():
    ...
    createTicket()
    placeNumbers()
    flashNumberLabel = canvas2.create_text(400,550, text = "Waiting for other to join...", font=("Chalkboard SE",30), fill="#3e2723")
...
def recivedMsg():
    ...
    while True:
        ...
        if(chunk in numbers and flashNumberLabel and not gameOver):
            ...
            canvas2.itemconfig(flashNumberLabel, text = chunk, font = ('Chalkboard SE', 60))
        elif('wins the game.' in chunk):
            gameOver = True
            canvas2.itemconfig(flashNumberLabel, text = chunk, font = ('Chalkboard SE', 40))
